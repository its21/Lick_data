...
def files():
    ...
    for filename in glob.glob(os.path.join(data_path, night+"????.fits")):
        ...
        try:
            ...
            elif obje == "HeHgCdArNe":
                arc_frames.append(filename)
                arc_targets.append(obje) #not in the head of the folder write for which target
                #we're taking the calibration lamp
            elif obje == "HAC9":
